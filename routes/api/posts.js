...
router.put('/love/:id', VerifyToken, async function(req,res){
    try {
        ...
        if(post.loves.filter(love=>love.user.toString()===req.user.id).length>0){
            return res.status(400).json({msg:'Post has already received love'});
        }
...
router.put('/unlove/:id', VerifyToken, async function(req,res){
    try {
        ...
        const remove = post.loves.map(love=>love.user.toString()).indexOf(req.user.id);
...
router.delete('/comment/:id/:comment_id', VerifyToken, async function(req,res){
    try {
        const post = await Post.findById(req.params.id);
